...
cast_path = output_dir / "external-project-workflow.cast"
gif_path = output_dir / "ac-3.gif"
...
steps = [
    (
        ...
        run_checked(
            ["bash", str(repo_root / "scripts" / "hosted-external-project-demo.sh")],
            env=env,
            cwd=repo_root,
        ),
    ),
...
write_cast(cast_path, steps, width=136, height=36)
subprocess.run(
    [
        "agg",
        "--theme",
