...
- Trigger: monthly review of whether `atxt` is ready to replace or augment the
  current `vhs`-backed human-reviewable recording path for `just mission`.
- Review the current proof contract first:
  - `just mission` should prove Port can host a minimal HTTP application, curl
    it successfully from the host, and surface a human-reviewable recording.
...
- Assess `atxt` readiness in the current repository environment:
  ...
  - verify it can capture the `just mission` app-hosting proof legibly in a
...
- Exit criteria:
  - if `atxt` is ready, create and activate a dedicated mission to migrate the
    canonical `just mission` recording path to `atxt`, then plan and implement
    the first scoped slice
