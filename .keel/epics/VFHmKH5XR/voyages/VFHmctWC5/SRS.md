...
- [SCOPE-06] Ingress, load balancer, or broader traffic-management features
  ...
  control plane.
- [SCOPE-07] Recorder migrations, proof UX work, downstream `infra`
  verification, or downstream guest-exec workarounds that bypass Port's
  cluster contract.
...
| Flux, Helm, and the Pulumi operator only require a real Kubernetes API surface plus the handed-off kubeconfig. | dependency | The voyage would need additional cluster prerequisites or bootstrap changes. |
...
- Treat downstream consumer verification as a follow-on handoff, not a blocking
  Port requirement in this voyage.
...
| ID | Requirement | Scope | Source | Verification |
|----|-------------|-------|--------|--------------|
| SRS-NFR-01 | Verification for this voyage must include live Kubernetes discovery, Flux install, and Helm install rather than Port-only surface checks. | SCOPE-03, SCOPE-04 | NFR-03 | board evidence + command proof |
